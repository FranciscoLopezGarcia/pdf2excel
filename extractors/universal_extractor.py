...
class UniversalBankExtractor:
    """Intenta TABLAS → TEXT; normaliza a columnas estándar."""
    ...
    def extract_from_pdf(self, pdf_path: str) -> pd.DataFrame:
        """Tablas → Texto. Devuelve DF normalizado."""
        # 1) Tablas
        try:
        ...
        try:
            ...
            if rows:
                ...
                return self._normalize_output(rows)
            log.info("Text method found no rows")
        except Exception as e:
    ...
    def _extract_from_tables(self, pdf_path: str) -> List[Dict]:
        ...
        for table in tables:
            ...
            # Heurística simple: buscar fila header
    ...
    def _map_columns(self, headers: List[str]) -> Dict[int, str]:
        ...
        for i, v in enumerate(h):
            ...
            elif 'déb' in v or 'debi' in v or 'cargo' in v: mapping[i]='debitos'
            elif 'créd' in v or 'credi' in v or 'abono' in v: mapping[i]='creditos'
    ...
    def _parse_table_row(self, row: pd.Series, colmap: Dict[int,str]) -> Dict:
        ...
        for i, val in enumerate(row):
            ...
            if not key: continue
            s = str(val).strip()
            if key=='fecha': tr['fecha']=self._normalize_date(s)
            elif key=='detalle': tr['detalle']=self._clean(s)
    ...
    def _parse_tx_line(self, line:str) -> Optional[Dict]:
        ...
        tr={'fecha':fecha,'detalle':detalle,'referencia':self._extract_ref(detalle),'debitos':'','creditos':'','saldo':''}
    ...
    def _skip(self,line:str)->bool:
        bad=[r'^\s*p[áa]gina', r'^\s*hoja', r'noticias', r'cbu:', r'situaci[oó]n impositiva', r'transporte',
    ...
    def _is_valid_transaction(self, tr:Dict)->bool:
        return bool(tr.get('fecha') and tr.get('detalle') and (tr.get('debitos') or tr.get('creditos') or tr.get('saldo')))
