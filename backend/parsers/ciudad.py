...
BANK_NAME = "CIUDAD"
DETECTION_KEYWORDS = ["BANCO CIUDAD", "RESUMEN DE CUENTA DOCUMENTACIÓN COMERCIAL", "BANCO CIUDAD DE BUENOS AIRES"]

class CiudadParser(BaseBankParser):
    BANK = BANK_NAME
    ...
    def parse(self, pdf_path: str, text: str = ""):
        ...
        patt = re.compile(
            rf"^(?P<fecha>{_DATE})\s+(?P<detalle>.+?)\s+(?P<deb>{_AMT})?\s+(?P<cred>{_AMT})?\s+(?P<saldo>{_AMT})(?:\s+(?P<ref>.+))?$",
            re.I
        )
        for ln in lines:
            if re.search(r"FECHA\s+CONCEPTO\s+D[EÉ]BITO\s+CR[EÉ]DITO\s+SALDO", ln, re.I):
                continue
            m = patt.match(ln)
            if not m:
                continue
